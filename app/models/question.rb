...
class Question < ApplicationRecord
  ...
  with_options presence: true do
    validates :question_category_id
    validates :content
    validates :explanation
    validates :number, uniqueness: { scope: :question_category_id }
  end
  ...
  class << self
    def import(file, question_category, overwrite)
      CSV.foreach(file.path, headers: true) do |row|
        question = question_category.questions.find_by(number: row['number'])

        if question
          overwrite_question_choices(question, row) if overwrite
        else
          create_question_choices(row, question_category)
        end
      end
    end
    ...
    def create_question_choices(row, question_category)
      question = question_category.questions.new(
        number: row['number'],
        content: row['question_content'],
        explanation: row['explanation']
      )
