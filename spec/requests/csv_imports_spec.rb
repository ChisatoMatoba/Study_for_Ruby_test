  ...
  describe 'POST /question_categories/:question_category_id/csv_imports' do
    ...
    context 'ユーザーが管理者の場合' do
      ...
      context 'ファイルが存在する場合' do
        it '正常にインポートされること' do
          expect { post question_category_csv_imports_path(question_category), params: { file: file, overwrite: true } }
        ...
        it '問題番号が足りないcsvの場合、エラーになること' do
          ...
          expect { post question_category_csv_imports_path(question_category), params: { file: missing_number_file, overwrite: true } }
            .to change(Question, :count)
            .by(1)
            .and change(Choice, :count)
            .by(4)
